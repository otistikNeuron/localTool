...
class BypassAutomation:
    ...
    def afc_op(self, op, *args):
        if self.afc_mode == "ifuse":
            ...
            # pymobiledevice3 implementation
            cmd = [self.pymobiledevice3_cmd, "afc"]
            
            if op == "push": 
                # Ensure we are pushing a file, not a directory
                if os.path.isdir(args[0]):
                    raise Exception(f"CRITICAL: Trying to push a directory '{args[0]}' instead of a file.")
                ...
                # Execute pull
                self._run_cmd(cmd + ["pull", remote_path, local_dest])
                
                # FIX: Check if it created a folder instead of a file
                if os.path.isdir(local_dest):
                    # The tool created a folder named 'local_dest' and put the file inside
                    # We need to find the file inside and move it out
                    files_inside = os.listdir(local_dest)
                    if files_inside:
                        actual_file = os.path.join(local_dest, files_inside[0])
                        # Move file out to a temp name
                        temp_move = local_dest + "_tmp"
                        shutil.move(actual_file, temp_move)
                        # Remove the accidental folder
                        os.rmdir(local_dest)
                        # Rename temp file back to intended name
                        os.rename(temp_move, local_dest)
...
class ModernApp(tk.Tk):
    ...
    def _create_ui(self):
        ...
        h.pack(fill="x")
        tk.Label(h, text="localTool Fixed", bg="#1a1a1a", fg="#00ffff", font=("Consolas", 18)).pack(side="left", padx=20, pady=15)
